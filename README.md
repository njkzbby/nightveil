...
|-------|---------------|--------------------------|
| L1 — IP/transport | IP protocol signatures | Standard TCP/HTTPS |
| L2 — TLS | JA3/JA4 TLS fingerprint | uTLS Chrome/Firefox/Safari ClientHello mimicry |
| L2 — TLS | SNI-based domain block | Configurable SNI; REALITY impersonates real site; CDN mode uses shared IPs |
| L3 — Application | Active probing — TLS certificate | **REALITY**: probes receive real certificate from target domain (google.com) |
| L3 — Application | Active probing — HTTP content | Fallback site returns real HTML content |
| L3 — Routing | ASN / IP mismatch detection | **REALITY**: server IP doesn't need to match SNI (probes see real site). **CDN mode**: origin IP belongs to Cloudflare ASN |
| L4 — Behavioural | Bidirectional symmetric traffic | XHTTP packet-up: upload and download are separate HTTP transactions |
...
| Go runtime | Go TLS stack fingerprint | **REALITY**: TLS handshake from real site, not Go. **CDN**: TLS terminated at CDN edge |
...
|------|-------------|------------------|
| L1 — Сигнатуры | Сигнатуры IP-протоколов | Стандартный TCP/HTTPS |
| L2 — TLS | JA3/JA4 TLS fingerprint | uTLS мимикрия Chrome/Firefox/Safari |
| L2 — TLS | Блокировка по SNI | Настраиваемый SNI; **REALITY** имитирует реальный сайт; **CDN** — общие IP |
| L3 — Active probing | Проверка TLS-сертификата | **REALITY**: зонды получают реальный сертификат (google.com) |
| L3 — Active probing | Проверка HTTP-контента | Fallback-сайт отдаёт реальный HTML |
| L3 — Маршрутизация | ASN/IP несоответствие | **REALITY**: зонды видят реальный сайт. **CDN**: IP принадлежит Cloudflare |
...
| Go runtime | Fingerprint Go TLS стека | **REALITY**: TLS от реального сайта. **CDN**: TLS на edge |
